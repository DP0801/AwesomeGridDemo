...
var sideMenu = function ($) {
    var loop = awef.loop, where = awef.where, select = awef.select;
    ...
    function build(opt) {
        ...
        function isScrollVis(elm) {
            var ctop = cont.scrollTop;
            var cbot = ctop + cont.clientHeight;
            ...
            $(window)
                .on('scroll resize',
                    function (e) {
                        ...
                        var docTop = $(document).scrollTop() + 20;

                        $('h2').each(function (i) {
                            var ht = $(this).offset().top;
                            if (ht > docTop) {
                                var res = i;
                                if (i > 0 && ht > docTop + $(window).height() * .7) {
                                    res--;
                                }
    ...
    var lnodes = null;
    function getMenuGridFunc(nodesOrXhr) {
        ...
        function addParentsTo(res, node, all) {
            if (node.ParentId) {
                ...
                if (!isFound) {
                    var parent = where(all, function (o) { return o.Id === node.ParentId; })[0];
                    res.push(parent);
        ...
        function buildMenuGridModel(gp) {
            ...
            // find selected
            var selectedItems = where(lnodes, function (o) {
                o.Selected = '';
                return isSelected(o);
            });
            ...
            if (selectedItems.length > 1) {
                ...
                var anchsli = where(selectedItems, function (o) {
            ...
            var regs = select(words, function (w) {
                return new RegExp(w, "i");
            });

            var regsl = regs.length;
            
            var result = where(lnodes, function (node) {
                var matches = 0;
            ...
            var rootNodes = where(result, function (o) { return !o.ParentId; });
    ...
    function menutree(o) {
        ...
        api.itmf = function (opt) {
            var content = '';
            var itm = opt.itm;
            var cebstr = opt.ceb ? api.ceb(itm.Url) : ''; // always add awe-ceb if url
            content += itm.Name;

            opt.clss += ' mnitm';
            if (opt.ceb && !itm.Url) { // no collapse expand class when url
                opt.clss += ' awe-ceb';
            }

            if (itm.Page) opt.clss += ' pmnitem'; // anchor on this page
            ...
            if (itm.Url && opt.ceb) {
                return '<div ' + attr + '>' + cebstr + '<a href="' + itm.Url + '" >' + content + '</a></div>';
            }

            return itm.Url ?
                '<a href="' + itm.Url + '" ' + attr + ' >'+ content + '</a>' :
                '<div ' + attr + '>' + cebstr + content + '</div>';
        };
    }
