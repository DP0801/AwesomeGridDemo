...
namespace AwesomeMvcDemo.Models
{
    public static class Db
    {
        ...
        static Db()
        {
            ...
            var foods = new[] {"Banana", "Cheesecake", "Hot Beverage", "Apple", "Oat meal", "French toast", "Pizza", "Apple Pie", "Shepherd's pie",
            "Salad", "Soup"};
            var locations = new[] { "Home", "University", "Restaurant", "Visit", "Diner", "Central Perk", "Tavern" };

            var restaurantNames = new[] { "McDowell's", "Chotchkie's", "Chili's", "Flingers", "The Rolling Donut", "Big Kahuna Burger", "City Wok", "Cluckin' Bell",
            ...
            for (var i = 0; i < 750; i++)
            {
                ...
                add(new Lunch
                {
                    Date = RndDate(),
                    Food = food,
                    FoodPic = food.Replace(" ", "").Replace("'", "") + ".jpg",
                    Person = person,
